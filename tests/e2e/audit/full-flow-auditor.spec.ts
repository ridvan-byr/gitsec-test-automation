...
test.describe('GitSec Grand Integration Flow (Toplu Entegrasyon Akışı E2E)', () => {
  ...
  test('Tüm platform adımlarını koştur ve Audit Log detaylarını doğrula', async ({ page }) => {
    ...
    // GitHub Install & Authorize tıklandıysa yeni bir bağlantı kurulmuştur
    const didGithubConnect = !isFinished ? true : false; // isFinished=true ise zaten bağlıydı, erken kapatıldı
    ...
    // Backup Başlatıldı Loglarını Kontrol Et
    await verifyAuditLogViaAPI(page, {
      category: 'Backup',
    ...
    // Restore Başlatıldı Loglarını Kontrol Et
    await verifyAuditLogViaAPI(page, {
      category: 'Restore',
    ...
    for (const provider of storageProvidersList) {
      ...
      try {
        ...
        console.log(`✅ ${provider.toUpperCase()} bağlantısı başarıyla oluşturuldu. Loglar kontrol ediliyor...`);

        // Ekleme Loglarını Kontrol Et
        await verifyAuditLogViaAPI(page, {
        ...
        // Şimdi depolama sağlayıcısını temizleyip silme logunu kontrol edelim
        console.log(`🧹 Depolama sağlayıcısı temizleniyor...`);
        await storagePage.navigateToStoragePage();
        await storagePage.cleanupExistingTestProviders(provider);

        // Silme Logunu Kontrol Et
        console.log(`🔍 Silme logu kontrol ediliyor...`);
        await verifyAuditLogViaAPI(page, {
          category: 'Storage Provider',
        ...
        // Herhangi bir açık popup varsa kapat
